    ...
    <div className="min-h-screen bg-[#fbfbfa] text-zinc-950">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 py-10 sm:px-8 sm:py-14">
        <div className="mb-10 max-w-2xl">
          ...
          <p className="mt-4 text-sm leading-7 text-zinc-500">上传 TXT 文件，确认章节目录后，即可选择故事中的角色重新走进这一章。</p>
        </div>

        {error && <div className="mb-5 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-xs text-red-700">{error}</div>}

        {!novel ? (
          <button onClick={() => inputRef.current?.click()} onDragOver={(event) => event.preventDefault()} onDrop={handleDrop} className="group flex min-h-[360px] w-full flex-col items-center justify-center rounded-[28px] border border-dashed border-zinc-300 bg-white px-8 text-center shadow-[0_20px_70px_rgba(0,0,0,.035)] transition hover:border-zinc-500">
            <input ref={inputRef} type="file" accept=".txt,text/plain" className="hidden" onChange={(event) => void handleFile(event.target.files?.[0])} />
            ...
            <span className="font-serif text-xl font-semibold">{isParsing ? "正在识别小说结构…" : "选择或拖入 TXT 小说"}</span>
            <span className="mt-3 max-w-sm text-xs leading-6 text-zinc-400">支持 UTF-8 与常见 GB18030 编码，单个文件不超过 5MB。小说原文保存在当前浏览器中。</span>
            <span className="mt-8 rounded-full bg-zinc-100 px-4 py-2 text-[11px] font-medium text-zinc-600">选择文件</span>
          </button>
        ) : (
          <div className="grid gap-6 lg:grid-cols-[340px_1fr]">
            <section className="rounded-[24px] border border-zinc-200/80 bg-white p-6 shadow-[0_18px_55px_rgba(0,0,0,.035)]">
              ...
              <h2 className="mt-8 break-words font-serif text-2xl font-semibold">{novel.title}</h2>
              <p className="mt-1 break-all text-xs text-zinc-400">{novel.fileName} · {novel.encoding}</p>
              <dl className="mt-8 grid grid-cols-2 gap-3 border-y border-zinc-100 py-5 text-center"><div><dt className="text-[10px] text-zinc-400">识别章节</dt><dd className="mt-1 font-serif text-xl">{novel.chapters.length}</dd></div><div><dt className="text-[10px] text-zinc-400">全书字数</dt><dd className="mt-1 font-serif text-xl">{novel.wordCount > 9999 ? `${(novel.wordCount / 10000).toFixed(1)}万` : novel.wordCount.toLocaleString()}</dd></div></dl>
            ...
            <section className="overflow-hidden rounded-[24px] border border-zinc-200/80 bg-white shadow-[0_18px_55px_rgba(0,0,0,.035)]">
              <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-5"><div><h2 className="font-serif text-lg font-semibold">确认章节目录</h2><p className="mt-1 text-[11px] text-zinc-400">点击章节查看原文开头</p></div><span className="text-[10px] text-zinc-400">本地规则识别</span></div>
              <div className="grid max-h-[420px] grid-cols-1 overflow-hidden sm:grid-cols-[minmax(220px,40%)_1fr]">
                <div className="overflow-y-auto border-r border-zinc-100 p-3">
                ...
                <div className="hidden overflow-y-auto p-6 sm:block"><p className="text-[10px] uppercase tracking-[.18em] text-zinc-400">正文预览</p><h3 className="mt-3 font-serif text-lg font-semibold">{novel.chapters[selectedChapter]?.title}</h3><p className="mt-5 whitespace-pre-line font-serif text-xs leading-7 text-zinc-500">{novel.chapters[selectedChapter]?.content.slice(0, 700)}{(novel.chapters[selectedChapter]?.content.length || 0) > 700 ? "…" : ""}</p></div>
              </div>
              <div className="flex flex-col gap-4 border-t border-zinc-100 bg-zinc-50/50 px-6 py-5 sm:flex-row sm:items-center sm:justify-between"><p className="text-[11px] leading-5 text-zinc-400">确认后保存原文，并开始真实解析第一章。</p><button onClick={() => void confirmImport()} disabled={isSaving} className="flex items-center justify-center gap-2 rounded-full bg-zinc-950 px-5 py-3 text-xs font-semibold text-white transition hover:bg-zinc-700 disabled:opacity-50">{isSaving ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : null}{isSaving ? "正在保存" : "确认并解析第一章"}<ArrowRight className="h-3.5 w-3.5" /></button></div>
            </section>
          </div>
        )}
      </main>
