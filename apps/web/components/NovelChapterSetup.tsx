...
import type { AiNarrativeMode, ChapterAnalysis, ImportedChapter, ImportedNovel, NovelDriveMode } from "../lib/import-novel/types";
...
export default function NovelChapterSetup({ storyId, chapterNumber }: { storyId: string; chapterNumber: number }) {
  ...
  const [aiNarrativeMode, setAiNarrativeMode] = useState<AiNarrativeMode>("faithful");
    ...
    <div className="min-h-screen bg-[#fbfbfa] text-zinc-950">
      ...
      <main className="mx-auto w-full max-w-6xl px-5 py-9 sm:px-8 sm:py-12">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_400px]">
          ...
          <aside className="lg:sticky lg:top-24 lg:self-start"><div className="rounded-[24px] border border-zinc-200 bg-white p-6 shadow-[0_24px_70px_rgba(0,0,0,.045)]"><div className="flex items-center justify-between"><p className="text-[10px] font-semibold uppercase tracking-[.18em] text-zinc-400">Reading mode</p><Sparkles className="h-3.5 w-3.5 text-amber-500" /></div><h2 className="mt-6 font-serif text-lg font-semibold">{driveMode === "ai" ? "AI 驱动叙事" : "沿原著阅读"}</h2><p className="mt-3 text-sm leading-7 text-zinc-600">{driveMode === "ai" ? (aiNarrativeMode === "free" ? "保留小说世界与人物基础，自由决定行动并探索不同走向。" : "选择围绕原文展开，人物、因果与主要剧情基本不偏离原作。") : "正文将严格按照导入文本的段落顺序展开。"}</p><div className="mt-5 grid grid-cols-2 rounded-full bg-zinc-100 p-1" role="radiogroup" aria-label="阅读驱动模式">{([{"id":"original","label":"原作驱动"},{"id":"ai","label":"AI 驱动"}] as const).map((mode) => <button key={mode.id} type="button" role="radio" aria-checked={driveMode === mode.id} onClick={() => setDriveMode(mode.id)} className={`rounded-full px-3 py-2 text-[11px] font-medium transition-all ${driveMode === mode.id ? "bg-white text-zinc-950 shadow-sm" : "text-zinc-400 hover:text-zinc-700"}`}>{mode.label}</button>)}</div>{driveMode === "ai" && <div className="mt-4"><p className="mb-2 text-[9px] font-medium tracking-[.12em] text-zinc-400">AI 叙事方式</p><div className="grid grid-cols-2 gap-2" role="radiogroup" aria-label="AI 叙事方式">{([{"id":"faithful","label":"遵循原作","description":"选项推进"},{"id":"free","label":"自由探索","description":"选项 + 输入"}] as const).map((mode) => <button key={mode.id} type="button" role="radio" aria-checked={aiNarrativeMode === mode.id} onClick={() => setAiNarrativeMode(mode.id)} className={`rounded-xl border px-3 py-3 text-left transition ${aiNarrativeMode === mode.id ? "border-zinc-900 bg-zinc-950 text-white" : "border-zinc-200 bg-white text-zinc-600 hover:border-zinc-400"}`}><span className="block text-[11px] font-semibold">{mode.label}</span><span className={`mt-1 block text-[9px] ${aiNarrativeMode === mode.id ? "text-zinc-400" : "text-zinc-400"}`}>{mode.description}</span></button>)}</div></div>}<div className="my-6 h-px bg-zinc-100" /><p className="text-[10px] text-zinc-400">当前身份</p><div className="mt-3 flex items-center gap-3 rounded-xl bg-zinc-50 p-3"><span className="grid h-9 w-9 place-items-center rounded-full bg-white font-serif text-xs shadow-sm">{selectedCharacter ? [...selectedCharacter.name][0] : ""}</span><div><p className="text-sm font-medium">{selectedCharacter?.name}</p>{selectedCharacter?.role && <p className="mt-0.5 text-[10px] text-zinc-400">{selectedCharacter.role}</p>}</div></div><p className="mt-5 rounded-xl border border-zinc-100 px-4 py-3 text-[10px] leading-5 text-zinc-400">{driveMode === "ai" ? (aiNarrativeMode === "free" ? "可选择建议行动，也可以输入自己真正想做的事。" : "只提供贴近原作的有限选项，不开放自由输入。") : "原作模式仅通过剧情选项推进，不提供自由输入。"}</p><button disabled={!selectedCharacter} onClick={() => router.push(`/story/${storyId}/chapter/${chapter.number}/play?role=${selected}&mode=${driveMode}${driveMode === "ai" ? `&narrative=${aiNarrativeMode}` : ""}`)} className="mt-5 flex w-full items-center justify-center gap-2 rounded-full bg-zinc-950 px-5 py-3.5 text-xs font-semibold text-white transition hover:bg-zinc-700 disabled:opacity-40">以 {selectedCharacter?.name || "所选角色"} 进入本章 <ArrowRight className="h-3.5 w-3.5" /></button></div></aside>
        </div>
      </main>
    </div>
  );
