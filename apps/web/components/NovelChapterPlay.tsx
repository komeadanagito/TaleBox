...
export default function NovelChapterPlay({ storyId, chapterNumber }: { storyId: string; chapterNumber: number }) {
  ...
  useEffect(() => {
    void getImportedNovel(storyId).then(async (storedNovel) => {
      ...
      const activeSession: ChapterSession = savedSession || {
        ...
        currentBlocks: storedAnalysis.blocks, currentChoices: storedAnalysis.choices, progress: 1, status: "playing", updatedAt: new Date().toISOString(),
      };
      if (!savedSession) await saveChapterSession(activeSession);
      setNovel(storedNovel);
      setSession(activeSession);
      setBlocks(activeSession.currentBlocks);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => revealAnchorRef.current?.scrollIntoView({ behavior: "smooth", block: allRevealed ? "start" : "center" }), 90);
    return () => window.clearTimeout(timer);
  }, [revealedCount, allRevealed]);

  const revealNext = () => {
    if (!allRevealed && !isAdvancing) setRevealedCount((count) => Math.min(count + 1, blocks.length));
  };
  ...
  const choose = async (choice: NovelChoice) => {
    ...
    try {
      ...
      setRevealedCount(1);
      previousRevealCountRef.current = 1;
    ...
    <div className="min-h-screen bg-[#e9e4da] text-[#312d27] selection:bg-[#d8cdbb]">
      ...
      <main style={{ maxWidth: `${readingWidth}px` }} className="mx-auto w-full px-3 py-5 transition-[max-width] duration-200 sm:px-8 sm:py-10">
        <article onClick={revealNext} className={`novel-reading-paper relative min-h-[calc(100dvh-7rem)] overflow-hidden rounded-sm border border-[#d8d0c2] px-6 py-10 shadow-[0_22px_70px_rgba(73,58,38,.13)] sm:px-14 sm:py-14 md:px-20 ${!allRevealed ? "cursor-pointer" : ""}`} aria-label="小说阅读页，点击空白处继续阅读">
          ...
          <header className="relative border-b border-[#d8d0c2]/80 pb-8 text-center"><p className="text-[9px] font-medium uppercase tracking-[.32em] text-[#9a8f80]">{novel.title} · Chapter {String(chapterNumber).padStart(2, "0")}</p><h1 className="mt-4 font-serif text-3xl font-semibold tracking-[.08em] text-[#2e2923] sm:text-4xl">{chapter.title}</h1><p className="mt-4 flex items-center justify-center gap-1.5 text-[10px] text-[#9a8f80]"><MapPin className="h-3 w-3" /> {analysis.location}</p><div className="absolute -bottom-px left-0 h-px w-full overflow-hidden bg-[#ded5c7]" title={`本章进度 ${session.progress}%`}><span className="block h-full bg-[#756956] transition-[width] duration-700 ease-out" style={{ width: `${session.progress}%` }} /></div></header>
          <div style={{ maxWidth: `${Math.min(1080, Math.max(620, readingWidth - 190))}px` }} className="relative mx-auto mt-10 pb-28 transition-[max-width] duration-200 sm:mt-12">
            <div className="mb-9 flex items-center justify-between border-b border-dashed border-[#d8d0c2] pb-5"><div className="flex items-center gap-3"><CharacterPortrait character={role} small /><div><p className="text-[9px] text-[#9a8f80]">本章扮演</p><p className="mt-0.5 font-serif text-sm font-semibold">{role.name}</p></div></div><p className="text-[9px] tracking-[.16em] text-[#aaa092]">第 {turn + 1} 幕</p></div>
            ...
            {!allRevealed && <button type="button" onClick={(event) => { event.stopPropagation(); revealNext(); }} className="novel-continue-mark mx-auto mt-12 flex items-center gap-2 text-[10px] tracking-[.14em] text-[#9a8f80]"><span>点击书页继续</span><span className="text-[8px]">▼</span></button>}
            {allRevealed && <section onClick={(event) => event.stopPropagation()} className="novel-block-reveal mt-14 border-t border-[#d8d0c2] pt-8">
          ...
          <footer className="absolute inset-x-0 bottom-5 flex items-center justify-center gap-4 text-[9px] text-[#aaa092]"><span className="h-px w-10 bg-[#d6ccbd]" /><span>{String(chapterNumber).padStart(2, "0")} · {turn + 1}</span><span className="h-px w-10 bg-[#d6ccbd]" /></footer>
        </article>
      </main>
