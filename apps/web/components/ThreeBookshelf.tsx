...
function ShelfBook({ story, index, isDeleting, isDimmed, onHover, onLeave, onSelect, onDeleteImported }: any) {
  ...
  const genre = isImported ? `${String(story.fileType).toUpperCase()} · 第 ${story.currentChapter || 1} / ${story.chapters.length} 章` : story.selectedGenre === "自定义" ? story.customGenre : story.selectedGenre;
  ...
  return (
    ...
    >
      ...
      {isImported && <span className="pointer-events-none absolute inset-x-0 bottom-0 z-30 h-1 bg-black/30"><span className="block h-full bg-white/90 transition-[width] duration-500" style={{ width: `${story.readingProgress || 0}%` }} /></span>}
...
export default function ThreeBookshelf({ stories, importedNovels, onSelectBook, onCreateNew, tab, deletingStoryId, onDeleteComplete, onDeleteImported }: ThreeBookshelfProps) {
  ...
  const preview = hoveredStory && (
    <div className="pointer-events-none absolute left-0 top-1/2 z-40 hidden w-40 -translate-y-1/2 text-left xl:block">
      ...
      <p className="mt-1 text-[10px] leading-relaxed text-zinc-400 line-clamp-3">{hoveredStory.fileType ? `${hoveredStory.author} · 读至第 ${hoveredStory.currentChapter || 1} 章 · 全书 ${hoveredStory.readingProgress || 0}%` : hoveredStory.framework?.worldView || "点击翻开故事，继续你的叙事旅程。"}</p>
